...
"""
   为了能让机器学习模型处理分子，我们使用MolGraph来把RDKit的分子转换成图数据结构。这个图包含了丰富的化学信息：
   每个原子用什么数字特征来表示（定义在 utils/mol_features.py）。
   每条化学键用什么数字特征来表示（定义在 utils/mol_features.py）。
   原子之间的连接关系是怎样的。
   以及一些为了能让图神经网络高效计算而需要的辅助索引信息（如反向边索引等）。
"""
