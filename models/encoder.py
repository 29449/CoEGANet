...
class EncoderFactory:
    ...
    Supports both original MPNEncoder and new GAT-based encoder.
    """
    
    @staticmethod
    def create_encoder(encoder_type: str, atom_fdim: int, bond_fdim: int, 
                     hidden_size: int, depth: int = 3, num_heads: int = 4,
                     dropout: float = 0.15, atom_message: bool = False,  
                     direction_mode: str = 'none',
                     alpha_input: str = 'none',
                     alpha_apply_mode: str = 'none') -> nn.Module:
        """
        Create encoder based on specified type.
        
        Args:
            encoder_type: 'mpn' for MPNEncoder, 'gat' for GATEncoder, 'unfolded_gat' for UnfoldedGATEncoder
            atom_fdim: Atom feature dimension
            bond_fdim: Bond feature dimension  
            hidden_size: Hidden layer size
            depth: Number of layers/depth
            dropout: Dropout rate
            atom_message: Whether to use atom-centered messages (for MPN)
            num_heads: Number of attention heads (for GAT)
            use_rbrs: Whether to use RBRS (for GAT)
            
        Returns:
            Encoder module
        """
        if encoder_type.lower() == 'mpn':
            return MPNEncoder(atom_fdim, bond_fdim, hidden_size, depth, dropout, atom_message, direction_mode)
        elif encoder_type.lower() == 'gat':
            return GATEncoder(atom_fdim, bond_fdim, hidden_size, depth, num_heads, dropout, atom_message, direction_mode,
                              alpha_input=alpha_input, alpha_apply_mode=alpha_apply_mode)
        else:
            raise ValueError(f"Unknown encoder type: {encoder_type}. Supported types: 'mpn', 'gat'")
