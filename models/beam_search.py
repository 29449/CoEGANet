...
class BeamSearch:
    ...
    def process_path(self, path, rxn_class):
        ...
        if 'rse_seq' not in path:
            path['rse_seq'] = []

        prod_mol = path['prod_mol']
        steps = path['steps'] + 1
        prod_tensors = self.model.to_device(path['tensors'])
        edit_logits, atom_state, state_scope, rxn_state = self.model.compute_edit_scores(
            prod_tensors, path['scopes'], path['atom_state'], path['state_scope'], path['rxn_state'])
        path['rse_seq'].append(rxn_state.detach().cpu())
