...
from models.encoder import Global_Attention, EncoderFactory
...
class Graph2Edits(nn.Module):
    ...
    def _build_layers(self) -> None:
        ...
        encoder_type = config.get('encoder_type', 'mpn')  # Default to MPN for backward compatibility
        ...
        self.encoder = EncoderFactory.create_encoder(
            ...
            atom_message=config.get('atom_message', False),
            direction_mode=config.get('direction_mode', 'none'),
            alpha_input=config.get('alpha_input', 'none'),
            alpha_apply_mode=config.get('alpha_apply_mode', 'none')
        )
    ...
    def compute_edit_scores(self, prod_tensors: Tuple[torch.Tensor],
        ...
        # Pass scopes to encoder (required for GAT encoder)
        if hasattr(self.encoder, 'forward') and 'atom_scope' in self.encoder.forward.__code__.co_varnames:
            # GAT encoder requires scopes
            a_feats = self.encoder(prod_tensors, mask=None, atom_scope=atom_scope, bond_scope=bond_scope)
        else:
            # MPN encoder (backward compatibility)
    ...
    def predict(self, prod_smi: str, rxn_class: int = None, max_steps: int = 9):
        ...
        while not done and steps <= max_steps:
            ...
            if idx.item() == len(edit_logits[0]) - 1:
                ...
                # idx 是扁平的全局 argmax 的位置
                flat_idx = idx.item()
                # 直接用整除与取模恢复 bond_idx, edit_idx
                bond_idx = flat_idx // self.bond_outdim
                edit_idx = flat_idx % self.bond_outdim

                # 额外保护：如果 bond_idx 超出范围（理论上不应），则退回 Terminate
                if bond_idx < 0 or bond_idx >= products.GetNumBonds():
                    print(f"[Warning] bond_idx out of range: {bond_idx}, terminating step.")
                    edits.append('Terminate')
                    done = True
                    break
                ...
                flat_idx = idx.item() - max_bond_idx  # 在 atom_logits 中的扁平位置
                atom_idx = flat_idx // self.atom_outdim
                edit_idx = flat_idx % self.atom_outdim

                if atom_idx < 0 or atom_idx >= products.GetNumAtoms():
                    print(f"[Warning] atom_idx out of range: {atom_idx}, terminating step.")
                    edits.append('Terminate')
                    done = True
                    break
