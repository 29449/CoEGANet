...
def plot_combined_accuracy(data, sheet_name, output_dir):
    ...
    colors = {
        ...
        'w/ α (atom-Attn)': '#1f77b4',       # 蓝色 - h-Attn
        'w/ α (m-Attn)': '#2ca02c',       # 绿色 - m-Attn
        'w/ α (m-atom-Attn)': '#d62728',     # 棕色 - out-Attn
    }
    ...
    if 'without' in sheet_name.lower():
        plt.ylim(0.25, 0.90)  # without sheet的数据范围
    else:
    ...
    output_path = os.path.join(output_dir, f'Attn_{sheet_name}.pdf')
...
def main():
    excel_path = "attn_1.xlsx"  # 替换为你的Excel文件路径
    output_dir = "results"
